...
def load_datasets(
    ...
    if args.dataset == "aligned":
    ...
    elif args.dataset == "trackfolder_fix":                                                          #### MINE
        ...
        #parser.add_argument("--source-augmentations", type=str, nargs="+")
        parser.add_argument(
            "--source-augmentations", type=str, default=[], nargs="+"
        )
        args = parser.parse_args()
...
class FixedSourcesTrackFolderDataset(UnmixDataset):
    ...
    def __getitem__(self, index):                                                  # who is index?
        #print(index)
        #print(index // self.samples_per_track)
        # select track
        ...
        if self.split == "train" and self.seq_duration:
            ...
            if self.random_chunks:
                ...
                #print("random chunksssssss")
                #start = random.uniform(0, min_duration - self.seq_duration)         #fix, it is now just randomized, just make it run across the every segment
                start = ((index%self.samples_per_track)/self.samples_per_track)*(min_duration - self.seq_duration)
            else:
                start = 0
        ...
        # pre-mixed musdb track
            ...
            audio_sources=[]
            audio, _=load_audio(track_path / self.target_file,start=0)
            audio_sources.append(audio)  #true target
            audio_mix, _=load_audio(track_path /"mixture.wav",start=0)
            audio_sources.append(audio_mix)       #just the mixture
...
if __name__ == "__main__":
    ...
    # Use train_sampler directly to iterate in batches
    total_training_duration = 0
    for i, (x_batch, y_batch) in enumerate(tqdm.tqdm(train_sampler)):
        # Compute the duration for each sample in the batch and add to the total
        batch_durations = [x.shape[1] / train_dataset.sample_rate for x in x_batch]
        total_training_duration += sum(batch_durations)
    
        # Save only if args.save is enabled
        if args.save:
            for j, (x, y) in enumerate(zip(x_batch, y_batch)):
                sample_index = i * args.batch_size + j
                torchaudio.save(f"test/{sample_index}_x.wav", x.T, train_dataset.sample_rate)
                torchaudio.save(f"test/{sample_index}_y.wav", y.T, train_dataset.sample_rate)
