...
def train(args, unmix, encoder, device, train_sampler, optimizer, max_bin, epoch):
    ...
    for x, y in pbar:
        ...
        #print(x.shape)
        if epoch==100 :
            torch.save(x, "x_100.pt")
        X = encoder(x)
        if epoch==100 :
            torch.save(X, "x_encoded_100.pt")
        #print("after encoder shape:")
        #print(X.shape)
        Y_hat = unmix(X,x)
        #print("after unmix shape:")
        #print(Y_hat.shape)
        #print("target shape:")
        #print(y.shape)
        Y = encoder(y)
        #print("target after encoder shape:")
        #print(Y.shape)
        """print(x)
        print(X)
        print(Y_hat)
        print(y)
        print(Y)"""
        # Crop spectrograms to the desired frequency range
        Y_cropped = Y[:, :, :max_bin, :]  # Keep only bins up to 16 kHz
        Y_hat_cropped = Y_hat[:, :, :max_bin, :]
        if epoch==100 :
            torch.save(Y_hat_cropped, "Y_hat_100.pt")
            torch.save(Y_cropped, "Y_100.pt")

        if epoch==300 :
            torch.save(Y_hat_cropped, "Y_hat_300.pt")
            torch.save(Y_cropped, "Y_300.pt")

        if epoch==700 :
            torch.save(Y_hat_cropped, "Y_hat_700.pt")
            torch.save(Y_cropped, "Y_700.pt")

        loss = torch.nn.functional.mse_loss(Y_hat_cropped, Y_cropped)                             #change MSE
        loss.backward()
        optimizer.step()
...
def valid(args, unmix, encoder, device, valid_sampler, max_bin):
    losses = utils.AverageMeter()
    unmix.eval()
    with torch.no_grad():
        for x, y in valid_sampler:
            ...
            Y_cropped = Y[:, :, :max_bin, :]  # Keep only bins up to 16 kHz
            Y_hat_cropped = Y_hat[:, :, :max_bin, :]
            loss = torch.nn.functional.mse_loss(Y_hat_cropped, Y_cropped)
            losses.update(loss.item(), Y.size(1))
        return losses.avg
...
def main():
    ...
    # Training Parameters
    parser.add_argument("--epochs", type=int, default=100)                                 #epochs and batch
    ...
    parser.add_argument(
        "--nb-workers", type=int, default=8, help="Number of workers for dataloader."       #workers
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    best_train_loss = float('inf')  # Set it to a very large value initially
    
    for epoch in t:
        t.set_description("Training epoch")
        end = time.time()
        train_loss = train(args, unmix, encoder, device, train_sampler, optimizer,max_bin, epoch)
        valid_loss = valid(args, unmix, encoder, device, valid_sampler,max_bin)
        ...
        if train_loss < best_train_loss:
            ...
            best_epoch_train = epoch  # Save the best epoch based on training loss
        ...
        print("save:")
        print(train_loss == best_train_loss)
        # save params
        params = {
            ...
            "best_epoch_trian": best_epoch_train, #mine
